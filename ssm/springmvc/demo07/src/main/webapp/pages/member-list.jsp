...
<div class="x-body">
  <div class="layui-row">
    <form class="layui-form layui-col-md12 x-so" action="<%=request.getContextPath()%>/account/FuzzyQuery">
      <input type="text" name="FuzzyQuery"  placeholder="请输入用户名" name="name" autocomplete="off" class="layui-input">
      <%--      <a href="#"><button class="layui-btn"  lay-submit="" lay-filter="sreach"><i class="layui-icon">&#xe615;</i></button></a>--%>
  ...
  <table class="layui-table x-admin">
    ...
    <c:forEach items="${findAll}" var="account">
      <tr>
        ...
        <td>${account.phone}</td>
        <td>${account.email}</td>
        <td>${account.levela}</td>
        <td class="td-status">
          <a onclick="x_admin_show('修改密码','member-password.jsp',600,400)" title="修改密码" href="<%=request.getContextPath()%>/pages/member-password.jsp?name=${account.name}">
            <i class="layui-icon">&#xe631;</i>
          </a>
            <%--        onclick="member_del(this,'要删除的id')"--%>
          <a title="删除"  href="<%=request.getContextPath()%>/account/delete/?name=${account.name}">
            <i class="layui-icon">&#xe640;</i>
          </a>
      </tr>
    </c:forEach>
